...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadInitialTemplates() {
        try {
            ...
            if (snapshot.exists()) {
                ...
                // Convert the object of templates to an array
    ...
    async function deleteTemplate(index) {
        ...
        if (confirm('Are you sure you want to delete this template? This cannot be undone.')) {
            try {
                const db = getDatabase();
                // Find the template key (template1, template2, etc.)
                const templateKey = `template${index + 1}`;
    ...
    async function saveTemplateChanges(formData) {
        try {
            ...
            if (!isNaN(editingIndex)) {
                ...
                templateKey = `template${editingIndex + 1}`;
                templateData.icon = window.templates[editingIndex].data?.icon || "📄";
            } else {
            ...
            if (!isNaN(editingIndex)) {
                ...
                window.templates.push({ data: templateData });
            }
