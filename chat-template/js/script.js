...
class PrudenceAIV2 {
    constructor() {
        ...
        this.templateSuggestions = [
            {
                keywords: ['presentation', 'lawyer', 'legal'],
                title: 'Legal Presentation Template',
                description: 'Create a professional legal presentation',
                icon: '⚖️',
                questions: [
                    'What type of legal presentation do you need?',
                    'Who is your target audience?',
                    'What are the key legal points you want to cover?',
                    'Do you need case references included?',
                    'What is your preferred presentation style?'
                ]
            },
            {
                keywords: ['presentation', 'business', 'company', 'corporate'],
                title: 'Business Presentation Template',
                description: 'Create a compelling business presentation',
                icon: '📊',
                questions: [
                    'What is the main purpose of this business presentation?',
                    'Who will be your audience?',
                    'What key metrics or data should be highlighted?',
                    'What is the desired outcome from this presentation?',
                    'Do you need to include financial projections?'
                ]
            },
            {
                keywords: ['presentation', 'education', 'teaching', 'school', 'university'],
                title: 'Educational Presentation Template',
                description: 'Create an engaging educational presentation',
                icon: '🎓',
                questions: [
                    'What subject are you teaching?',
                    'What age group or education level is your audience?',
                    'What are the key learning objectives?',
                    'Do you need interactive elements included?',
                    'How much time do you have for this presentation?'
                ]
            },
            {
                keywords: ['presentation', 'medical', 'healthcare', 'doctor'],
                title: 'Medical Presentation Template',
                description: 'Create a detailed medical presentation',
                icon: '🏥',
                questions: [
                    'What medical topic will you be presenting?',
                    'Is this for medical professionals or patients?',
                    'Do you need to include research data or clinical studies?',
                    'Are there specific medical visualizations you need?',
                    'What is the key medical information you want to convey?'
                ]
            },
            {
                keywords: ['presentation', 'tech', 'technology', 'software', 'IT'],
                title: 'Technology Presentation Template',
                description: 'Create a cutting-edge tech presentation',
                icon: '💻',
                questions: [
                    'What technology or product are you presenting?',
                    'Is your audience technical or non-technical?',
                    'Do you need to include code examples or technical specifications?',
                    'What are the key features or innovations to highlight?',
                    'Are you comparing this technology with alternatives?'
                ]
            }
        ];
...
**Please try again in a moment or use a different model.**`;
    ...
    showTemplateSuggestions() {
        const userInput = this.messageInput.value.trim().toLowerCase();
        ...
        if (!this.templateSuggestionsContainer) {
            this.createTemplateSuggestionsContainer();
        }
        
        // Clear previous suggestions
        this.templateSuggestionsContainer.innerHTML = '';
        
        // Only look for matching templates if there's input
        if (userInput) {
            // Find matching templates based on keywords
            const matchingTemplates = this.templateSuggestions.filter(template => {
            ...
            matchingTemplates.forEach(template => {
                ...
                templateElement.addEventListener('click', () => {
                    this.openTemplateQAModal(template);
                });
                
                this.templateSuggestionsContainer.appendChild(templateElement);
            });
        }
        
        // Show the container
        this.templateSuggestionsContainer.style.display = 'flex';
    }
    
    hideTemplateSuggestions() {
        if (this.templateSuggestionsContainer) {
            this.templateSuggestionsContainer.style.display = 'none';
        }
    }
    
    createTemplateSuggestionsContainer() {
        // Create container if it doesn't exist
        this.templateSuggestionsContainer = document.createElement('div');
        this.templateSuggestionsContainer.className = 'template-suggestions-container';
        this.templateSuggestionsContainer.id = 'template-suggestions-container';
        
        // Add CSS for template suggestions
        ...
        if (messageInputContainer) {
            ...
            messageInputContainer.appendChild(this.templateSuggestionsContainer);
        } else {
            // Fallback to body if container not found
            document.body.appendChild(this.templateSuggestionsContainer);
        }
    }
    ...
    openTemplateQAModal(template) {
        ...
        // Add questions with answer inputs
        template.questions.forEach((question, index) => {
            ...
            questionContainer.innerHTML = `
                <label style="font-weight: 600; color: #333;">${index + 1}. ${question}</label>
                <textarea class="qa-answer" data-question-index="${index}" style="
                    width: 100%;
                    min-height: 60px;
                    padding: 12px;
                    border: 1px solid #ddd;
        ...
        buttonsSection.innerHTML = `
            <button class="cancel-template-qa" style="
                padding: 10px 20px;
                border: 2px solid #ddd;
                background: white;
                color: #666;
                ...
                transition: all 0.2s;
            ">Cancel</button>
            <button class="generate-from-template" style="
                padding: 10px 20px;
                border: none;
                background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                color: white;
        ...
        // Add event listeners
        const closeBtn = modalContent.querySelector('.close-template-qa');
        const cancelBtn = modalContent.querySelector('.cancel-template-qa');
        const generateBtn = modalContent.querySelector('.generate-from-template');
        ...
        generateBtn.addEventListener('click', () => {
            const answers = [];
            modalContent.querySelectorAll('.qa-answer').forEach(textarea => {
            ...
            // Generate prompt based on template and answers
            const prompt = this.generatePromptFromTemplate(template, answers);
    ...
    generatePromptFromTemplate(template, answers) {
        ...
        let prompt = `Create a ${template.title.toLowerCase()} with the following requirements:\n\n`;
